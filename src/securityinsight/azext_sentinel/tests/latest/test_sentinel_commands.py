...
class SentinelClientTest(ScenarioTest):
    ...
    def test_sentinel_alert_rule_crud(self):
        ...
        self.cmd(
            "sentinel alert-rule create -g {rg} --alert-rule '{alert_rule}' --rule-id {alert_rule_name} --workspace-name {workspace_name}",
            checks=[
        ...
        self.cmd(
            "sentinel alert-rule list -g {rg} --workspace-name {workspace_name}",
            checks=[
                self.check("length(@)", 1),
        ...
        self.cmd(
            "sentinel alert-rule show -g {rg} --rule-id {alert_rule_name} --workspace-name {workspace_name}",
            checks=[
                self.check("name", "{alert_rule_name}"),
                self.check("type", "Microsoft.SecurityInsights/alertRules")
            ]
        ...
        self.cmd("sentinel alert-rule delete -g {rg} --rule-id {alert_rule_name} --workspace-name {workspace_name} --yes")

        self.cmd(
            "sentinel alert-rule template show -g {rg} --alert-rule-template-id 65360bb0-8986-4ade-a89d-af3cf44d28aa --workspace-name {workspace_name}",
            checks=[
        ...
        self.cmd("sentinel alert-rule template list -g {rg} --workspace-name {workspace_name}")
    ...
    @ResourceGroupPreparer(name_prefix="cli_test_sentinel_", location="eastus2")
    def test_sentinel_analytic_setting_crud(self):
        self.kwargs.update({
            "workspace_name": self.create_random_name("workspace-", 16),
